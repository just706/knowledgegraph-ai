...
# ------------------------- AI 解析 -------------------------
def explain_mistake(
    db: Session, user_id: int, mistake_id: int
) -> dict:
    """生成错题解析。优先 LLM，失败/无 key 降级本地模板。"""
    ...
    if settings.OPENAI_API_KEY:
        try:
            return {
                "explanation": _remote_explain(mistake),
                "mode": "llm",
            }
...
def _remote_explain(mistake: Mistake) -> str:
    import requests
    ...
    resp = requests.post(
        ...
        json={
            ...
            "messages": [
                {"role": "system", "content": "你是 KnowledgeGraph AI 的学习辅导老师。"},
...
def _local_explain(mistake: Mistake) -> str:
    ...
    lines = [
        ...
        "",
        "提示：当前为本地演示模式（未配置 LLM API Key），以上为记录回显。",
        "配置 OPENAI_API_KEY 后，将由 AI 生成诊断式讲解（错因分析 + 正确思路 + 防错方法）。",
    ]
    return "\n".join(lines)
